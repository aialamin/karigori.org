...
import {
  Star, ArrowLeft, MapPin, Clock, BadgeCheck, Phone,
...
/* WhatsApp brand icon (official SVG path) */
...
function Stars({ rating, size = 'md', interactive = false, value, onChange }) {
  ...
  const px = { sm: 14, md: 20, lg: 22, xl: 32 }[size] ?? 20;
      ...
      <div className="flex gap-1">
        {[1,2,3,4,5].map((i) => (
          <button key={i} type="button"
            ...
            <StarIcon filled={i <= (hov || value)} size={34} />
          </button>
        ))}
      </div>
      {(hov || value) > 0 && (
        <span className="text-sm font-semibold text-gray-600">{LABELS[hov || value]}</span>
      )}
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      ...
      <button type="submit" disabled={status === 'submitting'}
        ...
        {status === 'submitting' ? <Spinner label="Submitting…" /> : <><StarIcon filled size={16} className="shrink-0 brightness-[100] filter invert" /><span>Submit Review</span></>}
      </button>
    </form>
  );
}
...
export default function WorkerProfile() {
  ...
  const TABS = [
    ...
    { key: 'book',    label: 'Request',   shortLabel: 'Request',  icon: Send },
    { key: 'review',  label: 'Review',    shortLabel: 'Review',   icon: MessageSquare },
  ];
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 mt-5 flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 min-w-0 space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            {/* ── Tabs — stacked icon+text on mobile, side-by-side on sm+ ── */}
            <div className="border-t border-gray-100 flex">
              {TABS.map(({ key, label, shortLabel, icon: Icon }) => (
                <button key={key} onClick={() => setTab(key)}
                  className={`flex-1 flex flex-col sm:flex-row items-center justify-center gap-0.5 sm:gap-1.5 py-2.5 sm:py-3.5
                    text-[10px] sm:text-sm font-semibold transition-all border-b-2 min-w-0
                    ${tab === key
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
                      <StarIcon filled size={14} /> Recent Reviews
                    </h2>
                    <button onClick={() => setTab('reviews')}
          ...
          <div className="sticky top-20 space-y-4">
              ...
              <div className="p-5 space-y-3">
                {!callRevealed ? (
                  ...
                  <div className="space-y-2.5">
                    ...
                    <a href={`https://wa.me/880${worker.phone.replace(/^0/, '').replace(/-/g, '')}`}
                      ...
                      <WhatsAppIcon className="w-4 h-4 shrink-0" /><span>WhatsApp</span>
                    </a>
                  </div>
                )}
                ...
                <div className="flex items-center justify-center gap-4 pt-3 border-t border-gray-100 text-xs text-gray-400">
                  <span className="flex items-center gap-1">
                    <StarIcon filled size={12} />{(worker.rating || 0).toFixed(1)}
                  </span>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button onClick={() => setTab('review')}
                ...
                <StarIcon filled size={14} className="shrink-0 brightness-[10]" /><span>Review</span>
              </button>
            </div>
